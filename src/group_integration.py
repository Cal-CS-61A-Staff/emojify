...
class GroupIntegration(Integration):
    ...
    def responses(self):
        ...
        for match in re.finditer(TAG_REGEX, self._message):
            ...
            if not admins:
                continue
            users = users or requests.get("https://slack.com/api/users.list", params={"token": self._bot_token}).json()
            tags = []
            for member in users["members"]:
                for email, name in admins:
                    if member["profile"].get("email") == email or member["profile"].get("real_name_normalized") == name:
