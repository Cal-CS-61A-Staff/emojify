...
REGEX_TEMPLATE = r"<(https?://)?github\.com/Cal-CS-61A-Staff/berkeley-cs61a/issues/{}/?(\|[^\s|]+)?>"
...
class IssueIntegration(Integration):
    ...
    def message(self):
        ...
        for issue in self._issues:
            out = re.sub(
                REGEX_TEMPLATE.format(issue.path),
                "is/{}".format(issue.path),
                out,
            )
            out = re.sub(
                SHORT_REGEX_TEMPLATE.format(issue.path),
                r"<https://github.com/Cal-CS-61A-Staff/berkeley-cs61a/issues/{}|is/{}>".format(issue.path, issue.path),
                out,
            )
        return out
